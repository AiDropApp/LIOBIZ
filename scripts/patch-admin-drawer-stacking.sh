#!/usr/bin/env bash
# Fix mobile admin drawer: sidebar was trapped under backdrop (dash-shell z-index:1).
set -euo pipefail
...
for f in "$ROOT"/.next/static/css/*.css; do
  if grep -q 'dash-page' "$f" 2>/dev/null; then
    if grep -qF "$MARKER" "$f" 2>/dev/null; then
      echo "SKIP (stacking fix present) $f"
      continue
    fi
    BACKUP="${f}.bak-drawer-stacking-$(date -u +%Y-%m-%dT%H-%M-%S)"
    cp "$f" "$BACKUP"
    printf '%s' "$HOTFIX" >> "$f"
    echo "PATCHED $f"
    echo "BACKUP $BACKUP"
...
if [ "$patched" -eq 0 ]; then
  echo "NOTE: no new CSS bundles patched"
fi
