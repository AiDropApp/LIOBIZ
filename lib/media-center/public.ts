...
function imageDisplayUrl(ref?: MediaAssetRef | null, fallback = "/images/logo.png"): string {
  if (ref?.entryId && ref.kind !== "video") {
    return `/api/media/filesir/${ref.entryId}?thumb=1`;
  }
...
export function cardToPortfolioItem(card: MediaCard, categories: MediaCategory[]): PortfolioItem {
  const hasVideo = Boolean(card.video?.kind === "video" && card.video?.entryId);
  const videoSrc = hasVideo ? videoDisplayUrl(card.video) : undefined;
  const image = videoPosterUrl(card);
...
export function cardToBackstageItem(card: MediaCard): BackstageItem {
  const videoRef =
    card.video?.kind === "video" && card.video?.entryId ? card.video : null;
  const imageRef =
    card.image?.entryId && card.image.kind !== "video"
      ? card.image
      : card.cover?.entryId && card.cover.kind !== "video"
        ? card.cover
        : null;
...
export function cardToCreativePartner(card: MediaCard): CreativePartnerItem {
  const showcaseVideo = card.video?.kind === "video" ? card.video : null;
  const avatarRef = card.avatar?.entryId ? card.avatar : null;

  const avatarVideoSrc =
    avatarRef?.kind === "video" && avatarRef.entryId ? videoDisplayUrl(avatarRef) : undefined;

  let avatarSrc = "/images/logo.png";
  if (avatarRef?.entryId && avatarRef.kind !== "video") {
    avatarSrc = imageDisplayUrl(avatarRef, "/images/logo.png");
  }

  const videoRef = showcaseVideo || (avatarRef?.kind === "video" ? avatarRef : null);
  const hasVideo = Boolean(videoRef?.entryId);
